...
| P5.2 Option authority | **a** one `OptionRegistry` defines initiation/invariant/termination; **b** action server admission/feedback/result; **c** cancel/revoke then replace; **d** prevent old plan/candidate resurrection; **e** one `ExecutionState` owner. `tactics/options.py`, `option_executor_node.py`, `navigation/adapters.py`. | Causally ordered action/lease and cancel-race traces. | T21/T22, I11. |
